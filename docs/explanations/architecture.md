...
- **the renderer** (`somnia add`) — turns a Gutenberg id into per-chapter m4a
  files plus a semantic index, streaming chapter by chapter
...
`somnia add <gid>` streams: each chapter is written into the library folder and
indexed the moment it finishes, so listening can start minutes after picking a
book while the rest arrives overnight.
...
flowchart TD
  ...
  subgraph loop["for each chapter"]
    C["sentences — pysbd"] --> D["engine.render — Kokoro-82M<br>one sentence at a time"]
...
anything that can render one sentence satisfies the `TTSEngine` protocol.
...
WAL mode because three writers exist — an agent turn, the player, and a render
running in another process entirely — and a reader must never block on any of
them.
...
| `ingest` | The streaming pipeline that joins all of the above |
