...
| `SOMNIA_DATA_DIR` | `$XDG_DATA_HOME/somnia`, else `~/.local/share/somnia` | Holds `somnia.db`, and `streams/` — each book's chapters joined into the one file the page plays, which is a rebuildable cache and the largest thing in here. Created on startup if it is not there |
