...
# The model behind the conversation. claude-sonnet-5 is the alternative, and
# costs twice the wait and five times the money for no measured gain.
#SOMNIA_AGENT_MODEL=claude-haiku-4-5

# How hard it may think before answering: low, medium, high, xhigh or max.
# Only for models that have the dial — Haiku has not, so this does nothing
# unless the model above is changed.
